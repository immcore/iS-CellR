...
#cluster <- input$ClusterSel
if(!is.null(input$ClusterSel) && input$ClusterSel %in% Idents(scObject$val)){
    # find all markers of cluster 1
    ClusterMarkers <- FindMarkers(object = scObject$val, ident.1 = input$ClusterSel, test.use = "MAST")
    ClusterMarkers$cluster <- input$ClusterSel
    ClusterMarkers$val <- ClusterMarkers
    #print(x = head(x = cluster1.markers, n = 5))
} #else {
  #NAval <- NULL
  #ClusterMarkers$val <- NAval
  #ClusterMarkers <- NULL
#}
