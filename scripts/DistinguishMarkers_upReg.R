...
if(is.null(DownloadPlot$val$Upplot)){
  ...
  up.ident1 <- subset(df.volcano, p_val_adj<0.05 & avg_logFC>1)

  if(!is.null(up.ident1)){
    DownloadPlot$val$Upplot <- ggplotly(ggplot(data = up.ident1, aes(x= pct.1, y = pct.2, colour = pct.1)) + 
      geom_point(size = 2, mapping = aes(text=sprintf("Gene: %s", Gene))) + ggtitle(paste0("Up regulated genes in Cluster: ",input$MarkersCluster1)) +
